...
interface AnnouncementsStudentProps {
  announcements: AnnouncementsStudentType[];
}

const AnnouncementsStudent = ({ announcements }: AnnouncementsStudentProps) => {
  ...
  return (
    <article className="bg-white p-4 rounded-md min-h-[480px]">
      ...
      <section className="flex flex-col gap-4 mt-4 min-h-[260px]">
        {currentAnnouncements.map((announcement) => (
          ...
          >
            ...
            { announcement.message && (<p className="text-sm text-gray-400 mt-1">{announcement.message}</p>) }
          </article>
        ))}
      </section>
      <Pagination
